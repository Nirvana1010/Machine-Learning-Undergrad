...
if __name__ == '__main__':
    ...
    features_Y = np.reshape(features_Y, (1, -1))[0]

    m, n = np.shape(features_X)
    noise_X = np.zeros((m, n+101))
    noise_X[:, 0:147] = features_X.copy()
    for j in range(m):
        for i in range(147, 248):
            noise_X[j][i] = random.gauss(0, 0.3)
    for x in range(10):
        weight = fit(features=noise_X, labels=features_Y, iter_ratio=1, k=5, norm='2')
    ...
    ax.legend()
    # plt.show()
    plt.savefig('noise_100 relief-F score.png')
    ...
    ax.legend()
    # plt.show()
    plt.savefig('noise_100 relief-F auc.png')
